...
class CFW500ModbusNode(Node):
    ...
    def read_and_publish_registers(self):
        try:
            ...
            response = self.client.read_holding_registers(2, 1, unit=self.device_id)
            if not response.isError():
            ...
            response = self.client.read_holding_registers(3, 1, unit=self.device_id)
            if not response.isError():
            ...
            response = self.client.read_holding_registers(7, 1, unit=self.device_id)
            if not response.isError():
